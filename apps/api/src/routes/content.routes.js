...
function snapshotImageUrls(type,snapshot){return (imageFields[type]||[]).map(field=>snapshot?.[field]).filter(value=>typeof value==='string'&&value.trim());}
...
router.post('/trash/:revisionId/restore',async(req,res,next)=>{try{idOr400(req.params.revisionId);const revision=await ContentRevision.findOne({_id:req.params.revisionId,operation:'DELETE'});if(!revision)return res.status(404).json({error:'Trash item not found'});const Model=models[revision.type];if(!Model)return res.status(400).json({error:'Unknown content type'});const restored=await Model.create(revision.snapshot);await audit(req,'CREATE',revision.type,`${labels[revision.type]} restored`,restored._id,{restoredFrom:String(revision._id)});await ContentRevision.findByIdAndDelete(revision._id);res.status(201).json(restored);}catch(e){next(e);}});
router.delete('/trash/:revisionId/permanent',async(req,res,next)=>{try{idOr400(req.params.revisionId);const revision=await ContentRevision.findOne({_id:req.params.revisionId,operation:'DELETE'});if(!revision)return res.status(404).json({error:'Trash item not found'});const imageUrls=snapshotImageUrls(revision.type,revision.snapshot);for(const url of imageUrls){try{await deleteStorageObject(url);}catch(error){console.error(`Permanent trash storage cleanup failed for ${revision.type}:`,error.message);}}await ContentRevision.deleteMany({type:revision.type,resourceId:revision.resourceId});await audit(req,'DELETE_PERMANENTLY',revision.type,`${labels[revision.type]} permanently deleted`,revision.resourceId,{trashId:String(revision._id),storageDeleted:imageUrls.length});res.status(204).end();}catch(e){next(e);}});
